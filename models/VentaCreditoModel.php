...
class VentaCreditoModel
{
    ...
    public static function agregar($fecha_venta, $tipo_venta, $tipo_cliente, $cliente, $usuario, $total, $data, $aval, $plazo, $interes, $monto_total_interes)
    {
        ...
        try {
            ...
            // Calcular saldo inicial y primera cuota
            ...
            $primera_cuota = $monto_total_interes / $plazo; // Primera cuota
            $saldo = $monto_total_interes - $primera_cuota; // Saldo restante después de la primera cuota
            ...
            $cuenta_id = mysqli_insert_id($con);

            // Registrar primera cuota en historial de pagos
            $sql_historial = "INSERT INTO historialpagos (cuenta_id, fecha_pago, monto_pagado, interes_pagado, capital_abonado, saldo_restante)
                              VALUES ($cuenta_id, '$fecha_venta', $primera_cuota, 0, $primera_cuota, $saldo)";
            if (!mysqli_query($con, $sql_historial)) {
                throw new Exception("Error al registrar en historial de pagos: " . mysqli_error($con));
            }
